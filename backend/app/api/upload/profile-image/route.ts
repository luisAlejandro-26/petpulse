...
import { uploadProfileImage, UploadError } from '@/lib/storage'

export const runtime = 'nodejs'

const MAX_IMAGE_BYTES = 5 * 1024 * 1024 // 5MB en base64 crudo (aproximado)

// POST /api/upload/profile-image - Subir foto de perfil del usuario, devuelve la URL publica
export const POST = requireAuth(async (req: AuthedRequest) => {
  ...
  try {
    const url = await uploadProfileImage(id_user, image_base64, image_mime_type)
    return NextResponse.json({ url })
  } catch (error) {
    if (error instanceof UploadError) {
      return jsonError(error.message, error.status)
    }
    console.error('Error en /api/upload/profile-image:', error)
